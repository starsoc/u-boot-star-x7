...
NetSendUDPPacket(uchar *ether, IPaddr_t dest, int dport, int sport, int len)
{
	...
	net_debug("sending UDP to %08x/%pM\n", dest, ether);
...
static void receive_icmp(IP_t *ip, int len, IPaddr_t src_ip, Ethernet_t *et)
{
	...
	case ICMP_ECHO_REQUEST:
		net_debug("Got ICMP ECHO REQUEST, return %d bytes\n",
			ETHER_HDR_SIZE + len);
...
NetReceive(volatile uchar *inpkt, int len)
{
	...
	if (x < 1514) {
		...
		net_debug("VLAN packet received\n");
	...
	net_debug("Receive from protocol 0x%x\n", x);
	...
	case PROT_ARP:
		...
		case ARPOP_REQUEST:
			...
			net_debug("Got ARP REQUEST, return our IP\n");
			pkt = (uchar *)et;
		...
		case ARPOP_REPLY:		/* arp reply */
			...
			return;
		default:
			net_debug("Unexpected ARP opcode 0x%x\n",
			      ntohs(arp->ar_op));
			return;
		}
		break;
